...
root_output_dir="${root_dir}/techunits/imaging/work/ATTEND/achiodin/image_registration_pipeline/image_registration/output"

cd "$current_dir"

python "${current_dir}/shared/update_paths.py" \
    --input-dir "${input_dir}" \
    --output-dir "${root_output_dir}/registered_stitched_images" \
...
python "${current_dir}/image_registration/src/register_images.py" \
    --cur-logs-dir "${current_dir}/image_registration/logs/io/current/" \
    --mappings-dir "${root_output_dir}/mappings" \
    --registered-crops-dir "${root_output_dir}/registered_crops" \
